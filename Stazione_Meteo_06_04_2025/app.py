...
from frontend.emoji_frasi import *
...
def conversione(original_data):
    """
    Converte i dati meteo dal formato originale complesso a un dizionario semplificato.
    
    Args:
        original_data: I dati meteo originali (che possono essere una tupla o lista contenente un dizionario)
    
    Returns:
        Un dizionario con i dati meteo semplificati
    """
    # Estrai il dizionario dai dati originali
    if isinstance(original_data, (list, tuple)) and len(original_data) > 1:
        # Assumi che il dizionario sia il secondo elemento
        data_dict = original_data[1]
    else:
        # Se non è una lista/tupla o non ha il formato atteso
        return None
    
    # Crea il dizionario semplificato
    simplified_data = {
        'pressione': data_dict.get('barometer', 0),
        'temperatura': data_dict.get('outside_temp', 0),
        'umidità': data_dict.get('outside_humidity', 0),
        'precipitazione': data_dict.get('rain_rate', 0),
        'velocità media': data_dict.get('wind_speed', 0),
        'velocità raffica': data_dict.get('raffica', 0)
    }
...
def index():
    ...
    oggi=conversione(data)
    emoji_att=get_weather_emoji_and_description(oggi)
    emoji_d=get_weather_emoji_and_description(dom)
    emoji_dd=get_weather_emoji_and_description(dopodomani)
    emoji_ddd=get_weather_emoji_and_description(tregiorni)
# Da fare gestione giorni (data) e emojy da implementare
    return render_template('index.html', data=data, domani = dom, dopodomani = dopodomani, tregiorni = tregiorni, emoji_d=emoji_d, emoji_dd=emoji_dd,emoji_ddd=emoji_ddd, emoji_att=emoji_att)
...
def mezzanotte():
    while not shutdown_event.is_set():
        try:
            now = datetime.now()
            target_time = now.replace(hour=12, minute=26, second=30, microsecond=0)
            if now > target_time:
